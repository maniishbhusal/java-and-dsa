...
class CircularQueueImplementation {
    ...
    CircularQueueImplementation(int c) {
        ...
        size = c; // Set the size of the queue
        queue = new int[size]; // Initialize the array with the given size
    }
    ...
    void enqueue(int item) {
        if ((rear + 1) % size == front) {
            ...
            // If the queue is not empty, increment the rear pointer circularly and add the item
            rear = (rear + 1) % size;
            queue[rear] = item;
        }
    }
    ...
    void dequeue() {
        if (front == -1 && rear == -1) {
            ...
            // If there are multiple elements, remove the front element and increment the front pointer circularly
            System.out.println("Dequeued item: " + queue[front]);
            front = (front + 1) % size;
        }
    }
    ...
    void display() {
        if (front == -1 && rear == -1) {
            ...
            System.out.print(queue[i] + " "); // Print the last element
            System.out.println();
        }
    }
